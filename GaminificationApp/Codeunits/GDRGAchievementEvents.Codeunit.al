codeunit 50102 "GDRG Achievement Events"
{
    ...
    local procedure OnAfterCustomerPostingGroupInsert(var Rec: Record "Customer Posting Group"; RunTrigger: Boolean)
    var
        Processor: Codeunit "GDRG Achievement Processor";
        ContactCreatedMsg: Label 'Posting Group %1 created', Comment = '%1 = Posting Group Number';
    begin
        if not RunTrigger then exit;

        Processor.ProcessUserAction(
            CopyStr(UserId(), 1, 50),
            "GDRG Achievement Trigger Type"::PostingGroup,
            1,
            StrSubstNo(ContactCreatedMsg, Rec.Code)
        );
    end;
